...
export function PinterestAccountList({ accounts, currentId }: Props) {
  ...
  async function selectAccount(id: string) {
    ...
    startTransition(async () => {
      const res = await fetch("/api/connections/pinterest/select", {
        method: "POST",
  ...
  async function disconnect(id: string) {
    setBusyId(id);
    try {
      const res = await fetch(`/api/connections/pinterest/${id}`, {
        method: "DELETE",
      });
      if (res.ok) router.refresh();
    } finally {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base">
          Connected Pinterest accounts ({accounts.length})
        </CardTitle>
      </CardHeader>
          ...
          return (
            ...
            >
              ...
              <div className="flex gap-1">
                <a href="/api/connect/start/pinterest">
