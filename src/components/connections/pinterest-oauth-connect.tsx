...
export function PinterestOAuthConnect({
  connected,
  accountHandle,
  tokenExpiresAt,
  brandName,
  ...
  function startOAuth() {
    window.location.href = "/api/connect/start/pinterest";
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Production posting path. Uses Pinterest&apos;s OAuth flow and the official
          v5 API. This is what we&apos;ll use once Standard Access is approved.
        </CardDescription>
      </CardHeader>
        ...
        {connected ? (
          ...
          <>
            <ul className="space-y-1.5 text-sm text-muted-foreground">
              ...
              <li className="flex gap-2">
                <Check className="h-4 w-4 shrink-0 text-green-600" />
                Scopes requested: <code className="text-xs">user_accounts:read · boards:read · pins:read · pins:write</code>
              </li>
              <li className="flex gap-2">
                <Check className="h-4 w-4 shrink-0 text-green-600" />
                Each pin is published via{" "}
                <code className="text-xs">POST /v5/pins</code> after a human
                clicks Publish
              </li>
              <li className="flex gap-2">
                <Check className="h-4 w-4 shrink-0 text-green-600" />
